...
public class BeneficierCapaciteCroyantGuide extends CapaciteSpeciale {
	...
	public void effectuerCapaciteSpeciale(Player player, GameManager gameManager) {
		...
		if (card instanceof Believer) {
			//((Croyant) card).getCapaciteSpeciale().effectuerCapaciteSpeciale(joueur, partie); TODO Appeler la capacite speciale la carte card
		}else{
			//((GuideSpirituel) card ).getCapaciteSpeciale().effectuerCapaciteSpeciale(joueur, partie); TODO Appeler la capacite speciale la carte card
		}
	}
}
