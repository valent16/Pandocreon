...
public class SacrificeBeneficierCapaciteCroyantGuide extends Sacrifice{
	...
	public void effectuerSacrifice(Player player, GameManager gameManager) {
		...
		if (card instanceof Believer) {
			((Croyant) card).getCapaciteSpeciale().effectuerCapaciteSpeciale(player, gameManager); //TODO Appeler la capacite speciale la carte card
		}else{
			((GuideSpirituel) card ).getCapaciteSpeciale().effectuerCapaciteSpeciale(player, gameManager); //TODO Appeler la capacite speciale la carte card
		}
	}
}
